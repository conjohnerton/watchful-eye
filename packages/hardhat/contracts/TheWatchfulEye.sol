...
contract TheWatchfulEye is FlashLoanReceiverBase, Ownable {
    ...
    constructor(LendingPoolAddressesProvider _provider, address oneInch)
        public
    ...
    function removeWatchfulEye(address owner) public {
        require(
            watchfulEye.owner == owner,
    ...
    function executeOperation(
        ...
        // Swap aToken to reserve token on Aave
        {
            ...
            console.log(
                "Allowance of collateral",
                IERC20(collateralAsset).allowance(ownerOfDebt, address(this))
            );

            require(
                IERC20(collateralAsset).allowance(ownerOfDebt, address(this)) >=
                    totalCollateralCount,
                "The Watchful Eye does not have enough collateral allowance to complete the transaction."
            );
            require(
                IERC20(collateralAsset).balanceOf(ownerOfDebt) >=
                    totalCollateralCount,
                "There are not enough collateral funds to complete this transaction."
            );
            require(
                IERC20(collateralAsset).balanceOf(ownerOfDebt) >=
                    IERC20(collateralAsset).allowance(ownerOfDebt, address(this)),
                "Don't allow more than you can pay!!!!."
            );
        ...
        IERC20(reserveAsset).transfer(
            ownerOfDebt,
            IERC20(reserveAsset).balanceOf(address(this))
        );

        removeWatchfulEye(ownerOfDebt);
        return true;
    }
}
